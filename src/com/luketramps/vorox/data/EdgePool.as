...
package com.luketramps.vorox.data 
{
	...
	final public class EdgePool 
	{
		...
		private static var objPool:Vector.<Edge>;
		private static var objCounter:uint;
		private static var currentEdge:Edge;
		...
		public static function init(maxPoolSize:uint, growthSize:uint):void
		{
			...
			objPool = new Vector.<Edge> (MAX_SIZE, (GROW_SIZE == 0) ? true : false);
		...
		public static function getEdge():Edge
		{
			if (objCounter > 0)
			{
				currentEdge = objPool[--objCounter];
				currentEdge._sites = DictionaryPool.getDictFromSubpool ();
				return currentEdge;
			}
		...
		[Inline]
		public static function disposeEdge(e:Edge):void
		{
			objPool[objCounter++] = e;
			e.dispose ();
		}
		...
		public static function getEdgeFromSubpool():Edge
		{
			return currentPool.store (getEdge ());
		}
...
{
	public var references:Vector.<Edge>;
	public var numRefs:uint;
	public var id:uint;
	
	public function ReferenceStorage(size:uint, index:uint):void
	{
		references = new Vector.<Edge> (size);
		numRefs = 0;
		id = index;
	}
	
	public final function store (p:Edge):Edge
	{
	...
	public final function dispose():ReferenceStorage
	{
		for (var i:int = 0; i < numRefs; i++) 
			EdgePool.disposeEdge (references[i]);
