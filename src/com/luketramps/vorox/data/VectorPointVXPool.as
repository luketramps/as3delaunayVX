...
package com.luketramps.vorox.data 
{
	 ...
	 */
	final public class VectorPointVXPool 
	{
		// DEBUG
		...
		private static var objPool:Vector.<Vector.<PointVX>>;
		...
		public static function init(maxPoolSize:uint, growthSize:uint):void
		{
			...
			objPool = new Vector.<Vector.<PointVX>> (MAX_SIZE, (GROW_SIZE == 0) ? true : false);
			
			while (--i > -1)
				objPool[i] = new Vector.<PointVX> ();
				
			initSubpools();
		}
		
		public static function getObj():Vector.<PointVX>
		{
			if (objCounter > 0)
			{
			...
			while (--i > -1)
				objPool.unshift (new Vector.<PointVX> ())
		...
		[Inline]
		public static function disposeVector(d:Vector.<PointVX>):void
		{
			d.length = 0;
		...
		public static function getObjFromSubpool():Vector.<PointVX>
		{
...
{
	public var references:Vector.<Vector.<PointVX>>;
	public var numRefs:uint;
	public var id:uint;
	
	public function ReferenceStorage(size:uint, index:uint):void
	{
		references = new Vector.<Vector.<PointVX>> (size);
		numRefs = 0;
		id = index;
	}
	
	public final function store (p:Vector.<PointVX>):Vector.<PointVX>
	{
	...
	public final function dispose():ReferenceStorage
	{
		for (var i:int = 0; i < numRefs; i++) 
			VectorPointVXPool.disposeVector (references[i]);
