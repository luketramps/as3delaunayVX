...
package com.luketramps.vorox.data 
{
	 ...
	 */
	final public class VectorEdgePool 
	{
		// DEBUG
		...
		private static var objPool:Vector.<Vector.<Edge>>;
		...
		public static function init(maxPoolSize:uint, growthSize:uint):void
		{
			...
			objPool = new Vector.<Vector.<Edge>> (MAX_SIZE, (GROW_SIZE == 0) ? true : false);
			
			while (--i > -1)
				objPool[i] = new Vector.<Edge> ();
				
			initSubpools();
		}
		
		public static function getObj():Vector.<Edge>
		{
			if (objCounter > 0)
			{
			...
			while (--i > -1)
				objPool.unshift (new Vector.<Edge> ())
		...
		[Inline]
		public static function disposeVector(d:Vector.<Edge>):void
		{
			d.length = 0;
		...
		public static function getObjFromSubpool():Vector.<Edge>
		{
...
{
	public var references:Vector.<Vector.<Edge>>;
	public var numRefs:uint;
	public var id:uint;
	
	public function ReferenceStorage(size:uint, index:uint):void
	{
		references = new Vector.<Vector.<Edge>> (size);
		numRefs = 0;
		id = index;
	}
	
	public final function store (p:Vector.<Edge>):Vector.<Edge>
	{
	...
	public final function dispose():ReferenceStorage
	{
		for (var i:int = 0; i < numRefs; i++) 
			VectorEdgePool.disposeVector (references[i]);
